...
module.exports = function (extraField, vars) {
    ...
    while (!extra && extraField && extraField.length) {
        var candidateExtra = binary.parse(extraField).word16lu('signature').word16lu('partsize');
        ...
        if (candidateExtra.vars.signature === 0x0001) {
            ...
            if (vars.uncompressedSize === 0xffffffff) candidateExtra.word64lu('uncompressedSize');
            if (vars.compressedSize === 0xffffffff) candidateExtra.word64lu('compressedSize');
            if (vars.offsetToLocalFileHeader === 0xffffffff) candidateExtra.word64lu('offset');
            if (vars.diskNumber === 0xffff) candidateExtra.word32lu('disknum');
